...
const performSmartConversion = async (prompt: string) => {
  const response = await fetch(`${API_BASE_URL}/api/convert/smart`, {
    method: 'POST',
    headers: {
...
const convertDataFormat = async (data: string, targetFormat: string) => {
  const response = await fetch(`${API_BASE_URL}/api/convert/format`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ data, targetFormat }),
  });
...
const generateSalesFollowup = async (leadName: string, context: string) => {
  const response = await fetch(`${API_BASE_URL}/api/generate/sales-followup`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ leadName, context }),
  });
...
const DataConverter = () => {
  ...
  const handleConvert = async () => {
    ...
    setLoading(true);
    try { 
      setOutput(await convertDataFormat(input, target)); 
    } catch(e) { 
